...
function addTimeIn($idUser,$dateTime,$timeIn){
		connect();
		$sql = "insert into pointages(id_personnels,date_pointage,timeIn,timeOut) values(".$idUser.", '".$dateTime."','".$timeIn."','".$timeIn."')";
		doQuery($sql);
...
function addTimeOut($id,$timeOut){
		connect();
		$sql = "update pointages set timeOut='".$timeOut."' where id=".$id;
		doQuery($sql);
		commit();
}
...
function getDurationByUser($idPersonnel){
		connect();
		$sql = "SELECT SUM(TIMESTAMPDIFF(MINUTE, `timeIn`,`timeOut`))  as totalInMinute FROM `pointages`where id_personnels=".$idPersonnel;
		$res1 = doQuery($sql);
		$totalInMinute = 0;
		$nb1 = mysql_num_rows($res1);
		if( $nb1==0){
...
function updateColor($idPersonnel,$color,$idLog){
		connect();
		$sql = "update users set classCss='".$color."',idLogs=".$idLog." where id=".$idPersonnel;
		$res1 = doQuery($sql);
		commit();
}
?>